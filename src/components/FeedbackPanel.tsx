// 问题反馈的正文：提交表单 + 我的反馈记录。
//
// 抽成组件是因为它有两个入口：顶栏头像菜单里点开的弹窗，和 /main/feedback
// 这条路由。两处各写一份迟早会长歪——弹窗加了截图上传、页面忘了加之类。
// compact 只影响外框（弹窗自带标题栏与边框，不必再套一层）。
...
const FeedbackPanel: React.FC<{ compact?: boolean }> = ({ compact }) => {
  ...
  return (
    <div className={`feedback-page${compact ? ' is-compact' : ''}`}>
      {/* 弹窗里不再重复标题——Modal 自己有标题栏，再放一遍就是两层大字 */}
      {!compact && (
      ...
      <Card className="feedback-page__composer" title={compact ? undefined : '提交反馈'} bordered={!compact}>
      ...
      <Card className="feedback-page__history" title="我的反馈" bordered={!compact}>
