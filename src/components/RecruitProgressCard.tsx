...
import { getMyPreference, getMySchedule, MySchedule } from '@/api/interviewPreference';
...
const RecruitProgressCard: React.FC<Props> = ({ cycleId, resumeStatus }) => {
  ...
  if (scheduled) current = 4;                      // 已分配 → 查看安排
  ...
  const steps = [
    ...
    { title: '等待分配', description: scheduled ? '已分配' : (hasPreference ? '管理员安排中' : '') },
    {
      title: '面试安排',
      description: scheduled
        ? `${String(schedule!.interviewTime).replace('T', ' ').slice(0, 16)} · ${schedule!.deptName ?? ''} · ${schedule!.location ?? ''}`
        : '出结果后可在此查看',
    },
  ];
